...
import { Plus, Tag, Pencil, Trash2, CheckCircle, XCircle, ArrowLeft } from "lucide-react";
import { z } from "zod";
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            <Button variant="outline" size="sm" className="gap-1 shrink-0" onClick={() => window.history.back()} data-testid="button-voltar-segmentos">
              ...
              <Card key={s.id} className="hover:shadow-sm transition-shadow" data-testid={`card-segmento-${s.id}`}>
                <CardContent className="p-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => openEdit(s)} data-testid={`button-edit-segmento-${s.id}`}>
                      <Pencil className="w-3.5 h-3.5" />
                    </Button>
                    <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive hover:text-destructive"
                      onClick={() => deleteMutation.mutate(s.id)} data-testid={`button-delete-segmento-${s.id}`}>
      ...
      <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setEditing(null); form.reset(); } }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" className="gap-1 shrink-0" onClick={() => setOpen(false)} data-testid="button-voltar-form-segmento">
                <ArrowLeft className="h-4 w-4" /> Voltar
              </Button>
