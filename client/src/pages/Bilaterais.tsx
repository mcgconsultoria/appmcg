...
import { Plus, Users, Mail, Phone, Briefcase, Search, Pencil, Trash2, ArrowLeft } from "lucide-react";
import { z } from "zod";
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            <Button variant="outline" size="sm" className="gap-1 shrink-0" onClick={() => window.history.back()} data-testid="button-voltar-bilaterais">
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex gap-1">
                      <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => openEdit(b)} data-testid={`button-edit-bilateral-${b.id}`}>
                        <Pencil className="w-3.5 h-3.5" />
                      </Button>
                      <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive hover:text-destructive"
                        onClick={() => deleteMutation.mutate(b.id)} data-testid={`button-delete-bilateral-${b.id}`}>
      ...
      <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setEditing(null); form.reset(); } }}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" className="gap-1 shrink-0" onClick={() => setOpen(false)} data-testid="button-voltar-form-bilateral">
                <ArrowLeft className="h-4 w-4" /> Voltar
              </Button>
